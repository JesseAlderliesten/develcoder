...
- See [testthat::test_path()](https://testthat.r-lib.org/reference/test_path.html)
  on changing paths depending on how a check is run.
- On using temporary files, see `help("local")`, this
  [R-mail](https://stat.ethz.ch/pipermail/r-devel/2018-March/075783.html),
  and [withr::with_tempfile()](https://withr.r-lib.org/reference/with_tempfile.html):
...
also `tools::pkgVignettes(package = "<pkgname>")` for information about which
vignettes R finds.
...
Linking to a help page of a package: `help("<funcname>", package = "<pkgname>")`
...
`[<funcname>](../help/<funcname>)`, as proposed in a
...
(`pkgdown` recognises calls like `` `<funcname>()` `` and
...
the `_pkgdown.yml` file (run `tools::pkgVignettes(package = "<pkgname>")$names`
