...
`help(topic = "use_description", package = usethis)` and the
[usethis vignette](https://usethis.r-lib.org/articles/usethis-setup.html) for
details.
...
I do **not** follow the `usethis` practise of using `devtools::build_readme()`
...
### Package overview
`R/<pkg>-package.R` is used by `usethis` to list imported functions and can also
...
extension `.YAML`) to add a light-switch to the pkgdown-website (see the
