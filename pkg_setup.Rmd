...
bool_install <- !vapply(X = required_pkgs, FUN = requireNamespace, FUN.VALUE = logical(1), quietly = TRUE)
pkgs_install <- required_pkgs[bool_install]
if(length(pkgs_install) > 0L) {
  utils::install.packages(pkgs = pkgs_install, lib = NULL, dependencies = NA, quiet = FALSE)
}
```
...
[GNU FAQ](https://www.gnu.org/licenses/gpl-faq.html) for overviews of licenses.

A citation file makes clear how your package should be cited.
...
from the R-project folder (as done in a
[GitHub issue](https://github.com/r-lib/usethis/issues/312).
...
- you changed package `A` and want to check if package `B`, which depends on
  ...
  in `pkg_devel.R` on how to use it.
...
- If the package declares a dependency on a specific `R` version, it is useful
  ...
  `'- {os: ubuntu-latest,   r: '4.0.0'}'` to section `'matrix: config:'` to run
  `R` version 4.0.0.

### Further documentation
The file `progutils\.github\workflows\R-CMD-check.yaml` is a good example. See
the section about
[GitHub Actions](https://r-pkgs.org/software-development-practices.html#sec-sw-dev-practices-gha)
from [R packages](https://r-pkgs.org/) and the GitHub documentation about
