...
- `README_template.Rmd` for a `README.Rmd` file, to be put in the top-directory
  of `<pkg>`.
- `check-no-suggests.yaml` for a GitHub Actions workflow checking if the package
  code can be run if the suggested packages are **not** installed
...
To use these files, put the `README.Rmd` file in the top-directory of `<pkg>`.
The `yaml` files should be put in `<pkg>\.github\workflows` (see the section
`Automate checks` in the vignette
[package setup](https://jessealderliesten.github.io/develcoder/articles/pkg_setup.html)
